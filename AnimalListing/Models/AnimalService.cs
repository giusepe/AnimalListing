...
namespace AnimalListing.Models
{
    ...
    {
        public IObservableCache<Animal, string> Animals => _internalSourceCache;
        private readonly SourceCache<Animal, string> _internalSourceCache;

        public AnimalService()
        {
            _internalSourceCache = new SourceCache<Animal, string>(x => x.Name);
        }

        public IObservable<Unit> LoadAnimals()
        {
            return GenerateAnimals()
                .ToObservable()
                .Select(animal =>
                {
                    _internalSourceCache.AddOrUpdate(animal);
                    return Unit.Default;
                });
        }

        private IEnumerable<Animal> GenerateAnimals()
        {
            return new List<Animal>()
            {
                new Animal
                {
                    AnimalGroupName = "Bears",
